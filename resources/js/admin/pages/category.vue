       ...
       <div class="content">
			<div class="container-fluid">
				...
				<div class="_1adminOverveiw_table_recent _box_shadow _border_radious _mar_b30 _p20">
					<p class="_title0">Categroy <Button @click="uploadModel=true" v-if="isWritePermitted"><Icon type="md-add" /> Add Category</Button></p>
					...
					<div class="_overflow _table_div">
						<table class="_table">
							...
							<tr>
								...
								<!-- ITEMS -->
							<tr v-for="(category, i) in categroyList" :key="i" v-if="categroyList.length">
								<td>{{category.id}}</td>
								<td>
								...
								<td>
									...
									<Button type="error" size="small" @click="showDeletingModal(category,i)" :loading="categroyList.isDeleting" v-if="isDeletePermitted">Delete</Button>
				...
				<Modal
					...
					<div slot="footer">
						<Button type="default" @click="uploadModel=false">Close</Button>
...
export default {
	...
	methods: {
		async addCategory() {
			...
			console.log("image data",this.data.iconImage)
			const res = await this.callApi('post','app/create_category', this.data)
			if(res.status === 201){
				this.categroyList.unshift(res.data)
		...
		async editCategory() {
			...
			if(res.status == 200){
				this.categroyList[this.index].categoryName = this.editData.categoryName
	...
	async created() {
		...
		watch: {
			getDeleteModalObj(obj){
				if(obj.isDeleted){
					this.categroyList.splice(this.deletingIndex,1)
				}
			}
		}
